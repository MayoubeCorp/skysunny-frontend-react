...
const TossPayment = () => {
    ...
    useEffect(() => {
        ...
        async function initTossPayments() {
            ...
            try {
                ...
                console.log("[TOSS:init:loaded] v2 SDK 초기화 완료");
                ...
                alert("토스페이먼츠 v2 SDK 초기화 중 오류가 발생했습니다. 콘솔 로그를 확인하세요.");
            }
        }
    ...
    useEffect(() => {
        ...
        paymentMethodsWidget.updateAmount(order.amount);
        lastAmountRef.current = order.amount;
    }, [order.amount, paymentMethodsWidget]);
    ...
    const handlePayment = async () => {
        try {
            ...
            // 토스페이먼츠 권장: 결제 요청 전 orderId와 amount를 서버에 저장
            try {
                console.log("[TOSS] 결제 정보 서버 저장 중...", { orderId, amount });
                await window.updatePayment(orderId, {
                    orderId: orderId,        // 토스 권장: orderId 서버 저장
                    amount: amount,          // 토스 권장: amount 서버 저장
                ...
                console.log("[TOSS] 결제 정보 서버 저장 완료 ✅");
            } catch (updateErr) {
                console.error("[TOSS] 결제 정보 서버 저장 실패:", updateErr);
                // 토스 권장: 서버 저장 실패 시 결제 중단
            ...
            const payload = {
                orderId,                    // 토스 권장: 서버에 저장된 orderId 사용
                orderName: order.name,
                customerName: order.customerName,
                customerEmail: order.customerEmail,
                amount: amount,             // 토스 권장: 서버에서 검증된 amount 사용
                successUrl,
                failUrl,
            };
